//! SIGTERM / SIGINT → a readable fd, via the self-pipe trick.
//!
//! Identical to `nitro-server`'s `signals.rs`, and identical on purpose:
//! this is the second process in the tree that has to turn a signal into
//! a `poll`-able descriptor, and two different answers to that question
//! would be two different sets of bugs. `signal-hook` does the
//! async-signal-safe part — an empty `send` from the handler — which is
//! why the "pipe" is a `UnixDatagram` pair: an empty datagram is
//! readable, an empty write to a stream socket is not.
//!
//! The session takes `SIGINT` as well as `SIGTERM` because a developer
//! runs it from a terminal against `NITRO_BACKEND=fake`, and a Ctrl-C
//! that killed the supervisor without tearing down would leave four
//! processes and two sockets behind.
...
mod tests {
    ...
    fn install_drain_and_unregister() {
        ...
        if sigterm_blocked() {
            eprintln!("SIGTERM is blocked here; skipping the delivery check");
            return;
        }
