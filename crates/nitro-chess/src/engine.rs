...
//use bitintr::Popcnt;
use core::ops::Range;
// nitro-chess: the one change to this file. Upstream imports
// `num_traits::sign::signum`; the shim below is the same function for the
// two integer types the engine calls it on, so vendoring the engine does
// not add a crate to the tree (see `DEPENDENCIES.md`).
fn signum<T: Signum>(x: T) -> T {
    x.signum()
}
trait Signum {
    fn signum(self) -> Self;
