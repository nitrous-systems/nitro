...
Not crates: files from another project that live in this tree, with the
licence they came under. The rule is the same as for a dependency — it
has to earn its place, and where it came from has to be traceable to a
commit rather than to "upstream".

| asset | where | licence | why |
|---|---|---|---|
| **Bootstrap Icons** — 47 curated icons as SVG path data | `crates/nitro-icons/src/set.rs`, generated by `deploy/icons-import.sh`; upstream licence text at `crates/nitro-icons/LICENSE.bootstrap-icons` | MIT, © 2019–2024 The Bootstrap Authors | A symbolic icon set is artwork, not code: drawing 47 icons on a consistent 16-unit grid is a designer's job nobody should redo, and Bootstrap's set is MIT, has no external dependency at all (it is path data), and is already the de-facto reference for this style. Vendored as *path strings in a generated Rust file* rather than as SVG files, so nothing is parsed at runtime and the binary carries 43 KB of `&'static str`. Pinned to commit `6945b700`, recorded in the importer and in the generated file's header. |
| **The Salewski chess engine** — `engine.rs`, v0.6.0 | `crates/nitro-chess/src/engine.rs`; upstream licence text at `crates/nitro-chess/LICENSE.salewski-chess` | MIT, © 2015–2032 Dr. Stefan Salewski | `nitro-chess` is a third front end for the engine behind [`tiny-chess`](https://github.com/StefanSalewski/tiny-chess) and [`xilem-chess`](https://github.com/StefanSalewski/xilem-chess); writing a second chess engine is not what a GUI port is for. Copied from `xilem-chess` at commit `5c326d62` (the newer of the two copies). **One change**: upstream's `use num_traits::sign::signum` is replaced by a ten-line local shim, so the engine adds no crate. Held to upstream's lint allow-list rather than the workspace's, and `#[rustfmt::skip]`, so the file stays diffable against upstream. Its transposition table is 2 M entries allocated up front — **~560 MB RSS**, far outside the client budget; that is upstream's sizing, left as is. |

## M4-G: icons

**Zero new crates.** `zeno` — the path rasteriser — was already in the
tree as one of `swash`'s seven, so `nitro-icons` promoting it to a
**direct** dependency adds a line to a manifest and nothing to the
build.

| crate | used by | why | cost / notes |
|---|---|---|---|
| `zeno` | icons | 256× anti-aliased fill of an SVG path into an A8 mask, with the non-zero and even-odd rules — which is exactly and only what a symbolic icon needs. Already present via `swash`, which uses it for the same job one layer down. | **+0**. A direct dependency on a crate the tree already builds. |

One caveat is recorded here because it is a *correctness* trap rather
than a preference: **zeno 0.3.3 mis-parses SVG's implicit repeated arc
argument sets** (`a rx ry φ f1 f2 x y rx ry φ f1 f2 x y`, the command
letter written once), and `render_into` returns **no error** when it
does — it silently renders the prefix it managed to parse. 13 of our 47
icons are written that way upstream. `deploy/icons-import.sh` therefore
normalises every path to absolute, arc-free commands at import time, so
`set.rs` cannot contain the construct at all. See `docs/icons.md`.
